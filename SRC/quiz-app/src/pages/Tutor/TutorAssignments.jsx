...
const TutorAssignments = ({ isDark }) => {
  ...
  const fetchAssignments = async () => {
    try {
      // Tutors use getAllAssignments to see assignments they've created
      const response = await assignmentApi.getAllAssignments(currentPage, pageSize);
      console.log('Fetched tutor assignments:', response);
  ...
  const fetchPlayers = async () => {
    try {
      const response = await playerApi.getAllPlayers(1, 100);
  ...
  const filteredAssignments = assignments.filter(assignment => {
    const matchesSearch = assignment.quizTitle?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         assignment.userFirstName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         assignment.userLastName?.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesStatus = statusFilter === 'all' || assignment.status === statusFilter;
    const matchesQuiz = quizFilter === 'all' || assignment.quizId === quizFilter;
                  ...
                  <tr key={assignment.assignmentId} className={isDark ? 'hover:bg-gray-700' : 'hover:bg-gray-50'}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex items-center space-x-2">
                        <button
                          onClick={() => navigate(`/tutor/assignments/${assignment.assignmentId}`)}
                        ...
                        <button
                          onClick={() => navigate(`/tutor/assignments/${assignment.assignmentId}/edit`)}
          ...
          {(assignmentType === 'single' || assignmentType === 'bulk-users') && (
            <div className="mb-4">
              ...
              <select
                ...
                required
                className={`w-full px-3 py-2 border rounded-lg ${isDark ? 'bg-gray-700 border-gray-600' : 'bg-white border-gray-300'}`}
              >
